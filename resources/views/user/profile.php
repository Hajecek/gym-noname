...
<form class="card" method="post" action="<?= e(url('/user/profil/avatar')) ?>" enctype="multipart/form-data">
    <?= csrf_field() ?>
...
<form class="card" method="post" action="<?= e(url('/user/profil/avatar/smazat')) ?>" style="margin-top:16px"><?= csrf_field() ?><button class="btn btn-danger">Odstranit obrázek</button></form>
<form class="card" method="post" action="<?= e(url('/user/profil/email')) ?>" style="margin-top:16px">
    <?= csrf_field() ?>
    <h3>Změna e-mailu</h3>
...
<form class="card" method="post" action="<?= e(url('/user/profil/heslo')) ?>" style="margin-top:16px">
    <?= csrf_field() ?>
    <h3>Změna hesla</h3>
    <div class="field"><label>Současné heslo</label><input type="password" name="current_password" required></div>
    <div class="field"><label>Nové heslo</label><input name="password" type="password" required minlength="12"></div>
    <div class="field"><label>Potvrzení</label><input name="password_confirmation" type="password" required minlength="12"></div>
...
<div class="card" style="margin-top:16px">
        ...
        <table>
            ...
            <?php foreach ($sessions as $session): ?>
                <tr>
                    ...
                    <td>
                        <form method="post" action="<?= e(url('/user/profil/relace/' . $session['id'] . '/odhlasit')) ?>"><?= csrf_field() ?><button class="btn btn-danger">Odhlásit</button></form>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
    ...
    <form method="post" action="<?= e(url('/user/profil/odhlasit-vse')) ?>" style="margin-top:12px"><?= csrf_field() ?><button class="btn btn-secondary">Odhlásit všechna zařízení</button></form>
    <p style="margin-top:12px"><a class="btn btn-secondary" href="<?= e(url('/user/zabezpeceni/mfa')) ?>">Nastavit MFA</a></p>
</div>
<div class="card" style="margin-top:16px">
    <h3>Ochrana osobních údajů</h3>
    <a class="btn btn-secondary" href="<?= e(url('/user/profil/export')) ?>">Exportovat data</a>
    <form method="post" action="<?= e(url('/user/profil/vymaz')) ?>" style="display:inline"><?= csrf_field() ?><button class="btn btn-danger">Žádost o výmaz</button></form>
</div>
